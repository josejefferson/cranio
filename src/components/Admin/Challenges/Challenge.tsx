...
function Challenge({ challenge, handleEdit, handleDelete }: any) {
  ...
  challenge.courseNames =
    challenge.courseName?.length > 0 ? challenge.courseName.join(', ') : 'Todas as disciplinas'
  ...
  return (
    ...
    >
      <Ratio aspectRatio="16x9">
        <Image
          src={challenge.image || '/img/challenge-placeholder.svg'}
          alt={''}
          objectFit="cover"
          fallbackSrc="/img/challenge-placeholder.svg"
        />
      </Ratio>

      <Box p={3} display="flex" flexDirection="column" height="100%">
        <Text noOfLines={2} fontWeight={500} title={challenge.question}>
          {challenge.question}
        </Text>
        <Box my={1} title={challenge.topic}>
        ...
        <Box fontSize="sm">
          <HStack spacing={2} w="100%" color="blue.500" fontWeight={500}>
            ...
            <Box w="100%" title={challenge.courseNames}>
              <Text noOfLines={3}>{challenge.courseNames}</Text>
            </Box>
          </HStack>
          ...
          <HStack
            ...
            w="100%"
            color="red.500"
            hidden={challenge.alternatives.some((alternative: any) => alternative.correct)}
          >
            <Box>
              <MdWarning />
            </Box>
        ...
        <HStack spacing={1} mt={2} display="flex" flexGrow={1} alignItems="end">
          <Button variant="outline-primary" title="Editar" onClick={() => handleEdit(challenge)}>
            <MdEdit />
          </Button>
          <Button variant="outline-danger" title="Excluir" onClick={() => handleDelete(challenge)}>
            <MdDelete />
          </Button>
        </HStack>
      </Box>
